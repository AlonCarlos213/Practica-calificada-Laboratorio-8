    ...
    app.MapGet("/api/clientes/{clientid}/productos", async (int clientid, LINQExampleContext db) =>
        {
            var productosCliente = await db.Orderdetails
            ...
            if (!productosCliente.Any())
                return Results.NotFound($"No se encontraron productos para el cliente con ID {clientid}.");

            return Results.Ok(productosCliente);
        })
        .WithName("GetProductosPorCliente")
        .WithOpenApi();

    app.MapGet("/api/productos/{productid}/clientes", async (int productid, LINQExampleContext db) =>
        {
            var clientesPorProducto = await db.Orderdetails
                ...
                .Distinct() // Evita duplicados si un cliente compró el mismo producto varias veces
                .ToListAsync();

            if (!clientesPorProducto.Any())
                return Results.NotFound($"No se encontraron clientes para el producto con ID {productid}.");

            return Results.Ok(clientesPorProducto);
        })
        .WithName("GetClientesPorProducto")
        .WithOpenApi();
